...
class TransferListCreateAPIView(ListCreateAPIView):
    ...
    queryset = Transfer.objects.filter().select_related(
    ...
    def get_adjusted_account_balance_after_transfer_with_creating_transaction_history(
        self, accounts, transfers, account_type
    ):
        ...
        for transfer_data in transfers:
            ...
            if account_type == "source_account":
                ...
                transaction_history = TransactionHistory(
                    account_id=id,
                    transfer_account_id=transfer_data["destination_account"],
                    amount=amount,
                    old_balance=source_account_old_balance,
                    type="debit",
                )
                mapping_source_account_balances[id] = (
                    mapping_source_account_balances[id] - amount
                )
                ...
                if transfer_time > current_time:
                    ...
                    schedule_transfer.apply_async(
                        args=[transfer_data, source_account_old_balance],
                        eta=transfer_data["time"],
                    )
                ...
                if transfer_time <= current_time:
                    transaction_history = TransactionHistory(
                        account_id=id,
                        transfer_account_id=transfer_data["source_account"],
                        amount=amount,
                        old_balance=mapping_source_account_balances[id],
                        type="credit",
                    )
                    mapping_source_account_balances[id] = (
                        mapping_source_account_balances[id] + amount
    ...
    def bulk_update_accounts(self, request, account_type: str):
        account_ids = [
            item["source_account"]
            if account_type == "source_account"
            else item["destination_account"]
            for item in request.data
        ]
        accounts_from_db = Account.objects.filter(id__in=account_ids).values(
            "id", "balance"
        )
        account_ids_in_db = {}
        ...
        transfers = request.data
        adjusted_account_balance = self.get_adjusted_account_balance_after_transfer_with_creating_transaction_history(
            accounts_from_db, transfers, account_type
        )

        account_objects_to_update = [
            Account(id=key, balance=value)
            for key, value in adjusted_account_balance.items()
        ]
...
class TransactionHistoryListAPIView(ListAPIView):
    ...
    queryset = (
        ...
        .values("id", "account", "old_balance", "new_balance", "amount", "type")
        .order_by("created_at")
    )
