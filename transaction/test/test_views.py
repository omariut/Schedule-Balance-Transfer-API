...
class GetAllAccountsTest(TestCase):
    ...
    def setUp(self):
        ...
        Account.objects.create(name="Musa", contact="01787553320", balance=1000)
        Account.objects.create(name="Abdullah", contact="01787553321", balance=1000)
...
class GetSingleAccountsTest(TestCase):
    ...
    def test_get_valid_single_account(self):
        # get API response
        response = client.get(
            reverse("get_update_account", kwargs={"id": self.musa.id})
        )
        # get data from db
